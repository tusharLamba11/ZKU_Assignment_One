...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[9]); // line circom 25
while(Fr_isTrue(&expaux[0])){
Fr_add(&expaux[1],&lvar[1],&circuitConstants[9]); // line circom 27
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 27
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (2 * Fr_toInt(&lvar[1]))) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
...
Fr_add(&expaux[1],&lvar[1],&circuitConstants[9]); // line circom 28
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 28
{
uint cmp_index_ref = ((1 * Fr_toInt(&expaux[0])) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * ((2 * Fr_toInt(&lvar[1])) + 1)) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
hash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_geq(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 32
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_geq(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 32
